...
RSpec.describe Bls::Core::ApiClient do
  describe ".build" do
    it "sets the api" do
      api = Bls::Core::V1.new

      result = Bls::Core::ApiClient.build(api)
  ...
  describe "#execute_request" do
    it "sets the authorization bearer header" do
      ...
      api = Bls::Core::V1.new
      auth = Bls::Core::Authorization::Bearer.new(token: "1234")
      client = Bls::Core::ApiClient.new(api: api,
                                               authentication: auth)
      allow(Bls::Core::Authorization::Bearer).to receive(:new).
        and_return(auth)
      header = "Bearer 1234"
    ...
    context "the response code is 401" do
      it "raises a Bls::Core::AuthenticationError" do
        request = build_stubbed_post
        http_client = double
      ...
      it "resets the apis authentication" do
        ...
        begin
          client.execute_request(request: request, uri: request.uri)
        rescue Bls::Core::AuthenticationError
        end
  ...
  describe "#post" do
    ...
    context "when authorization is provided" do
      it "executes the request with authorization" do
        ...
        body = { test: "yes" }
        auth = Bls::Core::Authorization::Bearer.new(token: "1234")
        client = Bls::Core::ApiClient.new(authentication: auth)
